...
class MinesweeperAI():
    ...
    def add_knowledge(self, cell, count):
        ...
        for a in range(max(i - 1, 0), min(i + 2, self.height)):
            for b in range(max(j - 1, 0), min(j + 2, self.width)):
                if (i, j) != (a, b):
                    empty_cells.add((a, b))

        self.knowledge.append(Sentence(empty_cells, count))
    ...
    def mark_safe_or_mines(self):
        ...
        while times_to_iterate:
            ...
            for cell in self.safes:
                times_to_iterate = times_to_iterate + self.mark_safe(cell)
            for cell in self.mines:
                times_to_iterate = times_to_iterate + self.mark_mine(cell)
